...
We see that the x part of the tuple is a 4 dimensional numeric array and the y part is a Flux.OneHotVector.
...
> ``1=>16`` is the network input and output size.  The input size is 1 recalling that each data item fed into the network is really 28x28x1x128.  The output size is 16 meaning we’ll create 16 new feature matrices for every training digit in the batch.  
